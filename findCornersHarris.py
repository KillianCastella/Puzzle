...
def workimg(path):
    ...
    #écriture des images 
    if numberofBorder==1:
        cv2.imwrite("resultHarris/border/"+path,img)
    elif numberofBorder>1:
        cv2.imwrite("resultHarris/corner/"+path,img)
    elif numberofBorder == -1:
        cv2.imwrite("resultHarris/undefined/"+path,img)
    else:
        cv2.imwrite("resultHarris/inner/"+path,img)

def main():
    #création des dossiers si non existants
    if not os.path.exists("resultHarris/inner/"):
        os.makedirs("resultHarris/inner/")
    if not os.path.exists("resultHarris/border/"):
        os.makedirs("resultHarris/border/")
    if not os.path.exists("resultHarris/undefined/"):
        os.makedirs("resultHarris/undefined/")
    if not os.path.exists("resultHarris/corner/"):
        os.makedirs("resultHarris/corner/")
